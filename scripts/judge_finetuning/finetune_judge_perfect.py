...
Usage (ToolHop):
    python finetune_judge_perfect.py \
        --dataset toolhop \
        --data-path ${FORTE_ROOT}/toolhop_annotated_v1_remapped.json \
...
Usage (RL-only ablation — perfect plans only):
    python finetune_judge_perfect.py \
        --dataset toolhop \
        --data-path ${FORTE_ROOT}/toolhop_annotated_v1_remapped.json \
        --raw-data-path ${FORTE_ROOT}/ToolHop.json \
        --canonical-split ${FORTE_ROOT}/canonical_splits.json \
        --perfect-only \
        --output-dir models/judge-toolhop-perfectonly
...
PERFECT-ONLY MODE (RL ablation):
  With --perfect-only, train and val splits are restricted to gold plans only
  (those without any injected taxonomy error). The TEST split is intentionally
  kept unfiltered so downstream evaluation runs against the same test set as
  the full FORTE judge — enabling a clean head-to-head ablation comparison.
  A plan is considered "perfect" if EITHER:
    (a) its `error_type` field equals "none", OR
    (b) its annotation `quality_score` >= --min-quality-score (default 95).
...
# ─────────────────────────────────────────────────────────────────────────────
# Perfect-plan filter (for RL-only ablation)
# ─────────────────────────────────────────────────────────────────────────────

def _is_perfect_plan(item: Dict[str, Any], min_quality_score: int = 95) -> bool:
    """Return True iff this annotated item represents a gold/perfect plan.

    Decision priority:
      1. If the item has an `error_type` field, require it to equal "none".
         This is the most reliable signal because it comes directly from the
         contrastive augmentation pipeline.
      2. Otherwise, fall back to the annotation's `quality_score` field:
         require it to be >= min_quality_score (default 95, matching the
         paper's stated 95-100 band for gold plans).
      3. If neither field is available, return False (conservative: exclude
         rather than risk training on negatives we cannot identify).
    """
    if "error_type" in item:
        return item["error_type"] == "none"
    ann = item.get("annotation", {})
    qs = ann.get("quality_score")
    if qs is None:
        return False
    return qs >= min_quality_score
...
class JudgeDataset:
    ...
    def __init__(
        ...
        # Counters populated by load_and_split for split_info.json
        self.n_filtered_train = 0
        self.n_filtered_val = 0
    ...
    def load_and_split(self, split_ratios):
        ...
        # ── PERFECT-ONLY FILTER ────────────────────────────────────────────
        # Restrict TRAIN and VAL to gold plans only when --perfect-only is set.
        # TEST is intentionally left unfiltered so downstream evaluation runs
        # against the same examples as the full FORTE judge — enabling a
        # head-to-head ablation comparison.
        if self.perfect_only:
            n_train_before = len(train_items)
            n_val_before   = len(val_items)
            train_items = [it for it in train_items
                           if _is_perfect_plan(it, self.min_quality_score)]
            val_items = [it for it in val_items
                         if _is_perfect_plan(it, self.min_quality_score)]
            self.n_filtered_train = n_train_before - len(train_items)
            self.n_filtered_val   = n_val_before   - len(val_items)

            print(f"\n  ── --perfect-only filter applied ──")
            print(f"     min_quality_score = {self.min_quality_score}")
            print(f"     TRAIN: kept {len(train_items)}/{n_train_before} items "
                  f"(filtered {self.n_filtered_train} non-perfect)")
            print(f"     VAL:   kept {len(val_items)}/{n_val_before} items "
                  f"(filtered {self.n_filtered_val} non-perfect)")
            print(f"     TEST:  kept {len(test_items)}/{len(test_items)} items "
                  f"(UNFILTERED — for fair downstream eval)")

            # Recompute qid lists to reflect actual contributing qids in train/val
            contributing_train_qids = sorted({it["query_id"] for it in train_items})
            contributing_val_qids   = sorted({it["query_id"] for it in val_items})
            n_train_qid_dropped = len(self.train_qids) - len(contributing_train_qids)
            n_val_qid_dropped   = len(self.val_qids)   - len(contributing_val_qids)
            if n_train_qid_dropped or n_val_qid_dropped:
                print(f"     qids with no gold plan: "
                      f"{n_train_qid_dropped} dropped from train, "
                      f"{n_val_qid_dropped} dropped from val")
            self.train_qids = contributing_train_qids
            self.val_qids   = contributing_val_qids

            if len(train_items) == 0:
                raise RuntimeError(
                    "Perfect-only filter removed ALL training items. "
                    "Check that your annotated data contains items with "
                    "error_type=='none' or quality_score>=min_quality_score."
                )

        print(f"Split: {len(train_items)} train, {len(val_items)} val, {len(test_items)} test "
              f"({len(self.train_qids)}/{len(self.val_qids)}/{len(self.test_qids)} unique qids)")
...
def train_judge(
    ...
    print("\n" + "=" * 80)
    print(f"STARTING TRAINING  (dataset={dataset}, perfect_only={perfect_only})")
    print("=" * 80)
    trainer.train()
    ...
    split_info = {
        ...
        "perfect_only":         perfect_only,
        "min_quality_score":    min_quality_score if perfect_only else None,
        "n_filtered_train":     dataset_manager.n_filtered_train,
        "n_filtered_val":       dataset_manager.n_filtered_val,
    ...
    with open(f"{output_dir}/test_split.json", "w") as f:
        json.dump(
            {"metadata": {"split": "test",
                          "split_seed": SPLIT_SEED,
                          "dataset": dataset,
                          "perfect_only_training": perfect_only,
                          "note": ("Test split kept UNFILTERED even when "
                                   "perfect_only=True so it remains directly "
                                   "comparable to the full FORTE judge's test set.")},
    ...
    print("\n" + "=" * 80)
    print(f"TRAINING COMPLETE  ({dataset}, perfect_only={perfect_only})")
    print("=" * 80)
...
def main():
    ...
    # ── Perfect-only ablation flags ───────────────────────────────────────
    parser.add_argument(
        "--perfect-only",
        action="store_true",
        default=False,
        help="Train judge on GOLD plans only (no contrastive negatives). "
             "Filters train and val splits; test split is kept unfiltered "
             "for fair downstream comparison. Used for the RL-only ablation.",
    )
    parser.add_argument(
        "--min-quality-score",
        type=int,
        default=95,
        help="Fallback threshold for the perfect-only filter when items lack "
             "an `error_type` field. Items pass if annotation.quality_score "
             ">= this value. Default 95 matches the paper's gold-plan band.",
    )
