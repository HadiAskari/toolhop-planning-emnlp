#!/usr/bin/env python3
"""
ReAct Baseline for ToolHop / NESTFUL Tool Planning

Dataset-agnostic: pass --dataset {auto, toolhop, nestful}. Auto-detect reads
the `data_source` field from the parquet (produced by prepare_verl_rl_data.py).

Few-shot examples for ReAct are loaded from dataset_utils.py based on the
resolved dataset, so NESTFUL gets math/composition examples and ToolHop gets
multi-hop fact-retrieval examples.

Usage:
    # Auto-detect from parquet:
    CUDA_VISIBLE_DEVICES=3 python react_baseline.py \
        --model ${FORTE_ROOT}/planner_rl/checkpoints_grpo_filtered_qwen7b_nestful/global_step_72/actor \
        --test-parquet ${FORTE_ROOT}/planner_rl/data/verl_rl_nestful_full/test.parquet \
        --mode few-shot --full \
        --output qwen7b/react_few_shot_nestful_qwen-7b_results.json \
        --judge_url http://localhost:8002/v1/chat/completions

    # Explicit override:
    python react_baseline.py ... --dataset nestful ...
"""
...
def build_few_shot_prompt(question: str, tools: Dict[str, Any], few_shot_block: str) -> str:
    tools_str = format_tools(tools)
    return (
        f"{few_shot_block}\n\n"
        f"---\n\n"
...
def build_chat_messages(question: str, tools: Dict[str, Any],
                        mode: str, few_shot_block: str) -> List[Dict]:
    if mode == "zero-shot":
        user_content = build_zero_shot_prompt(question, tools)
    else:
        user_content = build_few_shot_prompt(question, tools, few_shot_block)
    return [
...
def _remap_gt_tool_name(nl_name: str, tools: Dict[str, Any]) -> str:
    """No-op for NESTFUL (GT already uses API names); remaps for ToolHop."""
    if nl_name in tools:
...
class ReactModel:
    ...
    def generate(self, question: str, tools: Dict[str, Any],
                 mode: str, few_shot_block: str,
                 temperature: float = 0.0,
                 max_new_tokens: int = 768) -> Tuple[str, str]:
        messages = build_chat_messages(question, tools, mode, few_shot_block)
...
def evaluate_react(
    ...
    results = []
    empty_plan_count = 0
    few_shot_block = get_few_shot_examples(dataset)

    for ex in tqdm(examples, desc=f"ReAct ({dataset}/{mode}) evaluation"):
...
def compute_stats(results: List[Dict], label: str) -> Dict:
    ...
    print(f"  {label}")
    print(f"{'='*W}")
    print(f"  Dataset   : {dataset_label(stats['dataset'])}")
...
def main():
    parser = argparse.ArgumentParser(description="ReAct baseline for ToolHop / NESTFUL tool planning")
    ...
    parser.add_argument("--dataset",             default="auto",
                        ...
                        help="Which dataset's few-shot examples to use. "
                             "'auto' detects from the parquet's data_source field.")
    ...
    # Resolve dataset (auto-detect or explicit)
    ...
    if args.perfect_only:
        ...
        stats = compute_stats(results, f"PERFECT-ONLY  ReAct ({args.mode}) — {dataset_label(dataset)}")
    ...
    if args.full:
        ...
        stats = compute_stats(results, f"FULL TEST SET  ReAct ({args.mode}) — {dataset_label(dataset)}")
