...
Usage:
    ...
    CUDA_VISIBLE_DEVICES=2 python lats_baseline.py \
        ...
        --full \
        --n 3 --k 2 \
        --output qwen7b/lats_results_qwen-7b-Instruct.json \
        --stats-output qwen7b/lats_stats_qwen-7b-Instruct.json \
        --judge_url http://localhost:8002/v1/chat/completions \
        --checkpoint qwen7b/lats_checkpoint_qwen7b.json
...
def evaluate_lats(model, examples, perfect_gt_by_qid, dataset: str, n=5, k=10, exploration_weight=1.0,
...
def compute_stats(results, label, n=5, k=10):
    ...
    stats = {
        "label": label,
        "dataset": results[0].get("dataset", "unknown"), "method": "LATS (MCTS + reflection)",
...
def main():
    ...
    if args.perfect_only:
        ...
        results = evaluate_lats(model, examples, perfect_gt_by_qid, dataset=dataset,
        ...
        stats = compute_stats(results, f"PERFECT-ONLY LATS (n={args.n}, k={args.k}) — {dataset_label(dataset)}", n=args.n, k=args.k)
    ...
    if args.full:
        ...
        results = evaluate_lats(model, examples, perfect_gt_by_qid, dataset=dataset,
                                ...
                                judge_max_tokens=args.judge_max_tokens, max_new_tokens=args.max_new_tokens,
                                return_raw=args.return_raw, checkpoint_path=ckpt)
        stats = compute_stats(results, f"FULL TEST SET LATS (n={args.n}, k={args.k}) — {dataset_label(dataset)}", n=args.n, k=args.k)
