...
Usage:
    ...
    python tool_planner_baseline.py \
        ...
        --full \
        --output qwen7b/tool_planner_results.json \
        --stats-output qwen7b/tool_planner_stats.json \
        --judge_url http://localhost:8002/v1/chat/completions

    # With explicit SimCSE embedding model (matches paper):
    CUDA_VISIBLE_DEVICES=3 python tool_planner_baseline.py \
        ...
        --full \
        --output qwen7b/tool_planner_qwen-7b-results.json \
        --judge_url http://localhost:8002/v1/chat/completions
...
def compute_stats(results: List[Dict], label: str) -> Dict:
    ...
    stats: Dict[str, Any] = {
        "label": label,
        "dataset": results[0].get("dataset", "unknown"), "method": "Tool-Planner (offline)", "n_examples": n,
...
def main() -> None:
    ...
    if args.perfect_only:
        ...
        stats = compute_stats(results, f"PERFECT-ONLY  Tool-Planner — {dataset_label(dataset)}")
    ...
    if args.full:
        ...
        print(f"  {len(examples)} examples")
        results = evaluate_tool_planner(model, clusterer, examples, **run_kwargs)
        stats = compute_stats(results, f"FULL TEST SET  Tool-Planner — {dataset_label(dataset)}")
