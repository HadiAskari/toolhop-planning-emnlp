...
Usage:
    ...
    CUDA_VISIBLE_DEVICES=7 python judge_server.py --model /path/to/judge --port 8002
    ...
    CUDA_VISIBLE_DEVICES=1 python gnn4taskplan_baseline.py \
        --model ${FORTE_ROOT}/planner_finetuning/checkpoints_planner_sft-llama-3b-nestful/global_step_150 \
        --test-parquet ${FORTE_ROOT}/planner_rl/data/verl_rl_nestful_full/test.parquet \
        --train-parquet ${FORTE_ROOT}/planner_rl/data/verl_rl_nestful_full/train.parquet \
        --mode graphsage --full \
        --gnn-epochs 20 --gnn-lr 1e-3 --gnn-hidden 256 \
        --output llama3b/gnn4tp_sage_llama-3B_finetuned_results.json \
...
def evaluate_gnn4taskplan(
    llm, embedder, retriever, mode, examples, perfect_gt_by_qid, dataset: str,
...
def main():
    ...
    eval_kwargs = dict(
        ...
        perfect_gt_by_qid=perfect_gt_by_qid, dataset=dataset, sgc_k=args.sgc_k,
    ...
    if args.perfect_only:
        ...
        stats = compute_stats(results, f"PERFECT-ONLY  GNN4TaskPlan-{args.mode.upper()} — {dataset_label(dataset)}")
    ...
    if args.full:
        ...
        print(f"  {len(examples)} examples")
        results = evaluate_gnn4taskplan(examples=examples, **eval_kwargs)
        stats = compute_stats(results, f"FULL TEST SET  GNN4TaskPlan-{args.mode.upper()} — {dataset_label(dataset)}")
