#!/usr/bin/env python3
"""
α-UMi Baseline for ToolHop Tool Planning — FIXED VERSION
...
Usage:
    ...
    CUDA_VISIBLE_DEVICES=5 python judge_server.py \
        --model ${FORTE_ROOT}/judge_finetuning/models/judge/merged \
        --port 8001 --batch-size 32 --batch-timeout-ms 200
    ...
    for line in plan_text.split("\n"):
        ...
        try:
            ...
            if var_match:
                ...
                # identifier so we don't false-positive on text like
                # "Step 0: do the lookup with tool(...)" (no such pattern in
...
def main():
    ...
    if args.perfect_only:
        ...
        stats = compute_stats(results, f"PERFECT-ONLY  α-UMi ({mode_label})")
    ...
    if args.full:
        ...
        stats = compute_stats(results, f"FULL TEST SET  α-UMi ({mode_label})")
